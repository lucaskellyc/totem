...
export class BulbEffect {
  ...
  applyTo(root, spec) {
    ...
    root.traverse((node) => {
      ...
      // Reference a bulb material so the cast light can share its scroll fade.
      if (!fadeMat) fadeMat = node.material;
      this.bulbs.push({
  ...
  update(t) {
    // Totem stashes a per-material scroll fade (0..1) on userData.emissiveFade;
    // multiply the blink on/off by it so bulbs dim off-focus like everything else.
    for (const b of this.bulbs) {
      const on = ((t * this.hz + b.phase) % 1) < 0.5;
      const fade = b.material.userData.emissiveFade ?? 1;
      b.material.emissiveIntensity = (on ? b.onIntensity : 0) * fade;
    }
    for (const l of this.lights) {
      const on = ((t * this.hz + l.phase) % 1) < 0.5;
      const fade = l.fadeMat?.userData.emissiveFade ?? 1;
      l.light.intensity = (on ? l.onPower : 0) * fade;
    }
  }
}
